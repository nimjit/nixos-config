      ...
      (dolist (file (directory-files dir t "\\.md$"))
        (let* ((grade    (my/dash--fm-from-file file "grade"))
          ...
          (when (or (null grade) (string-empty-p (or grade "")))
            (let* ((diff (and deadline (not (string-empty-p deadline))
                               (my/dash--date-ts
                                (concat (my/dash--normalize-date deadline)
                                        " 12:00:00"))))
                   (diff (when diff (/ (- (float-time diff) (float-time)) 86400)))
...
(defun my/render-md-table (lines)
  "Convert markdown pipe-table LINES to box-drawing unicode (like neovim render_md_table)."

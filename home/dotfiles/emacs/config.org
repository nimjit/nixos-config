...
(defun my/dash-messages ()
  "Open WhatsApp via nchat in a vterm. Reuses existing *nchat* buffer."
  (interactive)
  (let ((bufname "*nchat*"))
    (if (get-buffer bufname)
        (switch-to-buffer bufname)
...
(defun my/dash--uni-planning ()
  "Read # Planning from Uni MOC.md; return org-table-compatible lines."
  (let ((moc (expand-file-name "Uni MOC.md" my/dash-uni))
    ...
    (mapcar (lambda (line)
              (if (string-match "^|[[:space:]]*[-:]" line)
                  ;; Convert markdown separator row to org-table separator
                  (let ((cols (length (split-string (string-trim line "|" "|") "|"))))
                    (concat "|" (string-join (make-list cols "---") "+") "|"))
                line))
            lines)))
...
(defun my/dashboard ()
  ...
  (let* ((buf      (get-buffer-create "*Dashboard*"))
    ...
    (with-current-buffer buf
      ...
      ;; Local keybindings — evil-local-set-key overrides evil's own M, u, etc.
  ...
  (let* ((buf         (get-buffer-create "*Uni*"))
    ...
    (with-current-buffer buf
      (let ((inhibit-read-only t))
        ...
        (insert (format "   [[file:%s][Open Uni MOC — full planning table]]\n\n"
                        (expand-file-name "Uni MOC.md" my/dash-uni)))
        (insert "\n")
...
(defun my/news-view ()
  ...
  (let* ((countries '(("nl"     . "🇳🇱 Netherlands")
         ...
         (all-entries
          (let (es)
            (maphash (lambda (_id entry) (push entry es))
                     (elfeed-db-entries elfeed-db))
            es)))
    (with-current-buffer buf
      (let ((inhibit-read-only t))
        (erase-buffer)

          ...
          (let* ((title   (or (elfeed-entry-title entry) "(no title)"))
            ...
            (if (and path (file-exists-p path)
                ...
                (insert-image (create-image path 'jpeg nil
                                            :max-width 220 :max-height 124) " ")
              (insert (propertize (make-string 22 ?░) 'face 'shadow)))
            (insert "\n")
            (insert " " dot " "
            ...
            (insert sep "\n")))
        (insert (propertize " RET play  o browser  c category  u refresh  q quit\n"
                            'face 'shadow))
        (read-only-mode 1)))
...
(defun my/yt--highlight-current ()
  "Put hl-line overlay on the title row of the current entry."
...
Each completed download cancels any pending redraw and reschedules one
for 1.5 s later — so a burst of downloads causes only one redraw."
...
(defun my/yt--refresh ()
  "Fetch new entries; redraw when elfeed finishes writing to the DB."
  (interactive)

...
#+begin_src emacs-lisp
  (use-package elfeed
    ...
    ;; Fetch all feeds at startup (0 delay) and then every 2 hours.
    (run-with-timer 0 (* 2 3600) #'elfeed-update))
...
(defun my/yt--draw-list ()
  "Erase and redraw *YouTube List* in single-column layout. Returns the buffer."
        ...
        (dolist (entry my/yt--all-entries)
          (let* ((title   (or (elfeed-entry-title entry) "(no title)"))
            ...
            (if (and path (file-exists-p path)
                ...
                (insert-image (create-image path 'jpeg nil
...
(defun my/yt--highlight-current ()
  "Highlight the current entry's title row; show keybind footer below it."
        ...
        (save-excursion
          ...
          (let ((eol (line-end-position)))
            ...
            (overlay-put my/yt--footer-overlay 'after-string
                         (propertize "\n  RET play  o browser  c category  u refresh  q quit"
                                     'face 'shadow))))))))
